...
class Footer extends React.Component {
  ...
  storePos = (msPos) => {
    apiObjSetStorage(this.props.curPlay,"mSec",msPos)
  }

  restorePos = (obj) => {
    apiObjGetStorage(obj,"mSec").then((value) => {
      if (value==null){
        value=0;
      }
...
// BUG FIX !!!
    ...
    if (!soundPlayerBugFix){
      ...
      if (this.state.curDur !== cur.duration){
        apiObjSetStorage(this.props.curPlay,"mSecDur",cur.duration)
  ...
  handleVideoDuration = (dur) => {
    ...
    apiObjSetStorage(this.props.curPlay,"mSecDur",durMSec)
...
console.log(url)
  ...
  render() {
    const {curPlay, curPos, isPaused, isWaitingForPlayInfo} = this.props;
    ...
    if ((curPlay!=null)) {
      ...
      if ((curEp!=null)&&(curEp.filename!=null)) {
        locURL = getLocalMediaFName(curEp.filename);
      } else if ((curSerie!=null)&&(curSerie.curPath!=null)) {
        locURL = getLocalMediaFName(curSerie.curPath);
      }
      ...
      if (!videoFound) {
       ...
       const extStr = locURL.match(fileExtensionPattern)[0];
